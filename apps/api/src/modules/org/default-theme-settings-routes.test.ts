...
describe('PATCH /api/v1/organizations/:orgId/default-theme-settings (Story 16.4 AC-1/4/5/7/8/9)', () => {
  let app: TestApp
  ...
  it('AC-1 edge (Red Team Round 2): the 404 body is byte-for-byte identical for a nonexistent org id vs. an org that exists but belongs to another tenant', async () => {
    ...
    expect(resNonexistent.headers['content-length']).toBe(resOtherTenant.headers['content-length'])
  })
