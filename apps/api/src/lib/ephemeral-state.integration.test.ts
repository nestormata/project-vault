...
describe('ephemeralState — Story 20.8 integration (real Postgres, real crypto, real RLS)', () => {
  beforeAll(async () => {
  ...
  it('AC-16: oversized key/value rejected before any DB call; AC-11 cap is per-org, unaffected by another org', async () => {
    ...
    await expect(bindAndRun(orgId, () => host.set('k', 'v'.repeat(20 * 1024), 60))).rejects.toThrow(
      EphemeralStateValidationError
    )
  })
