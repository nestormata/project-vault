...
describe('checkOrgAuthorization — Story 23.11 ambient-context AC7/AC8/AC9 (live Postgres, real request lifecycle)', () => {
  beforeAll(async () => {
  ...
  it("AC8: two concurrent, interleaved requests for different orgs never see each other's ambient context", async () => {
    ...
    ;(app as JwtTestApp).route({
      ...
      handler: async (request: FastifyRequest) => {
        ...
        const auth = (request as FastifyRequest & { authContext?: { userId: string } }).authContext
        const outcome = await checkOrgAuthorization(
