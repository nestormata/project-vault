...
describe('buildNotificationOriginatorHost — Story 36.1 real-Postgres/RLS integration (AC2, AC3, AC4, AC5)', () => {
  ...
  describe('AC5 Pre-Mortem — concurrency', () => {
    it('two racing calls at the cap boundary may both succeed (accepted best-effort overshoot), neither corrupts the count nor crashes', async () => {
      ...
      expect(rows.length).toBeGreaterThanOrEqual(NOTIFICATION_ORIGINATOR_RATE_LIMIT_MAX_PER_WINDOW)
      expect(rows.length).toBeLessThanOrEqual(NOTIFICATION_ORIGINATOR_RATE_LIMIT_MAX_PER_WINDOW + 1)
    }, 30_000)
  })
})
