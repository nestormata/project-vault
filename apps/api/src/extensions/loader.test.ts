...
describe('loadExtension — valid package (AC-2)', () => {
  ...
  it('Story 41.1 Task 3: buildHostServices() actually wires monitoring with all nine methods (createServiceEndpoint added)', async () => {
    let capturedHost: Record<string, unknown> | undefined
    const importFn = vi.fn().mockResolvedValue({
      default: {
        manifest: VALID_MANIFEST,
...
describe('loadExtension — fatal-equivalent failure logging (Task 4)', () => {
  ...
  it('warns on every load using the explicit above-host rollback escape', async () => {
    ...
    // 39.1 AC8, Story 41.1, Story 56.1 AC1/AC4, and Story 40.1 AC1/AC2/AC3/AC10 — host
    // EXTENSION_API_VERSION is now 3.19.0 (see manifest.ts's EXTENSION_API_VERSION doc comment
    // for the full version-bump history); '3.20.0' is the above-host, same-major
    ...
    const aboveHostApiVersion = '3.20.0'
    const importFn = vi.fn().mockResolvedValue({
      default: {
