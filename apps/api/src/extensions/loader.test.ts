...
describe('loadExtension — fatal-equivalent failure logging (Task 4)', () => {
  ...
  it('warns on every load using the explicit above-host rollback escape', async () => {
    ...
    // 39.1 AC8, and Story 41.1 — host EXTENSION_API_VERSION is now 3.17.0 (see manifest.ts's
    // EXTENSION_API_VERSION doc comment for the full version-bump history); '3.18.0' is the
    ...
    const aboveHostApiVersion = '3.18.0'
    const importFn = vi.fn().mockResolvedValue({
      default: {
