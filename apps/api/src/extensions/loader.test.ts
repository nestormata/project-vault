...
describe('loadExtension — fatal-equivalent failure logging (Task 4)', () => {
  ...
  it('warns on every load using the explicit above-host rollback escape', async () => {
    ...
    // 20.11 AC1, Story 34.1 AC1/AC9, Story 35.1 AC1, Story 36.1 AC1/AC6, Story 37.1 AC1.3, and
    // Story 39.1 AC8 — host EXTENSION_API_VERSION is now 3.16.0 (see manifest.ts's
    // EXTENSION_API_VERSION doc comment for the full version-bump history); '3.17.0' is the
    ...
    const aboveHostApiVersion = '3.17.0'
    const importFn = vi.fn().mockResolvedValue({
      default: {
