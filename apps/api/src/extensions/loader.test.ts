...
describe('loadExtension — fatal-equivalent failure logging (Task 4)', () => {
  ...
  it('warns on every load using the explicit above-host rollback escape', async () => {
    ...
    // 58.1, Story 20.12, and Story 20.13 — host EXTENSION_API_VERSION is now 3.23.0 (see
    // manifest.ts's EXTENSION_API_VERSION doc comment for the full version-bump history);
    // '3.24.0' is the above-host, same-major escape-eligible version. Kept one minor version
    // above whatever EXTENSION_API_VERSION currently is — a future bump must move this value
    // forward again the same way this story just did, or this test silently stops exercising the
    // above-host path once EXTENSION_API_VERSION catches up to a stale hardcoded value.
    const aboveHostApiVersion = '3.24.0'
    const importFn = vi.fn().mockResolvedValue({
      default: {
