...
test.describe('J12 — bounded/scoped credential sharing', () => {
  test('AC-2/AC-9: default share excludes the sensitive field; naming it explicitly includes it', async ({
    ...
    const defaultValueCode = recipientPage.locator('code').last()
    await expect(defaultValueCode).toContainText(USERNAME_VALUE)
    await expect(defaultValueCode).not.toContainText(PASSWORD_VALUE)
    ...
    const explicitValueCode = recipientPage2.locator('code').last()
    await expect(explicitValueCode).toContainText(PASSWORD_VALUE)
