<script lang="ts">
  ...
  import type { HealthDashboardService } from '@project-vault/shared'

  let { data } = $props()

  function statusClass(status: HealthDashboardService['status']): string {
    switch (status) {
      case 'healthy':
