...
## Writing a real capability gate

This is a test fixture, not a template. Everything a real, non-fixture extension needs before
writing an `onCheckCapability()` — the no-caching contract, the fail-closed rule, why `message`
cannot be localized, why quota is out of scope, and what `GET /api/v1/capabilities` does and does
not expose — now lives in
[`docs/extensions/authoring.md`](../../docs/extensions/authoring.md#capability-gates), along with
the recovery procedure for a broken gate. Start there.
