...
- `credentials.current_version_id` — explicit FK to `credential_versions.id`. Flipped atomically inside the same rotation-completion transaction that already writes the new `credential_versions` row, updates `rotations.status`, and confirms `rotation_checklist_items` (per the existing compound-transaction Reliability NFR). Never derived via `MAX(created_at)` — a derived pointer is ambiguous during the window between a new version's insert and the rotation's confirmation. **Correction (Story 5.6):** this description does not match the shipped rotation path — `currentVersionId` is written only by the multi-field-edit path (`insertVersionAndSetCurrent()`/Story 13.2) and is never read or written by `initiateRotation()`/`completeRotation()`/`promoteRotation()`/`retireRotation()`. The rotation path's actual "current version" selection is a query over `credential_versions` (`ORDER BY promoted_at DESC, version_number DESC WHERE promoted_at IS NOT NULL AND purged_at IS NULL AND abandoned_at IS NULL`, Story 5.6 AC-1), not this column. This is a documented pre-existing inconsistency, not fixed by Story 5.6 — wiring `currentVersionId` into the rotation path is out of scope, flagged as a larger, separate refactor for a future story.
...
**License field rationale:** `packages/extension-api/package.json` carries an explicit `"license": "AGPL-3.0-or-later"` field, inherited from the repo root (set by Story 14.0). `packages/extension-api` inherits the repo's AGPLv3 license; a private extension package importing it as a workspace dependency does not trigger AGPLv3's network-copyleft obligations because it isn't redistributing this package, only consuming its types/contracts in-process. This makes `packages/extension-api` the only workspace package with an explicit `license` field — every sibling package (`packages/shared`, `packages/crypto`, `packages/db`, `packages/agent`, etc.) omits the field. Note this is not automatic tooling-level inheritance (npm/pnpm does not propagate a root `package.json`'s `license` field onto workspace children, and no per-package `LICENSE` files exist); those packages are simply covered by the repository's single root `LICENSE` file and AGPLv3 posture by convention, with nothing machine-readable at the package level. `packages/extension-api` gets the explicit field as a deliberate one-off given its cross-license-boundary consumption story, not an inconsistency to "fix" by removing it or adding it everywhere.
...
|---|---|---|---|
| A. Stay private, in-repo only (this story) | Keep `private: true`, add explicit `license` field, document consumption path and future trigger | Zero *new build/distribution* risk, zero new runtime surface, matches Story 14.1's "no publish pipeline" scope boundary; defers real publish-readiness work to whenever it's actually needed — the licensing rationale itself still carries the caveat below and is not a legal guarantee | ✅ Yes |
...
**Scope of this rationale — read narrowly:** the "no redistribution, no network-copyleft trigger" argument above is analyzed only for the in-monorepo `workspace:*` case. It is *not* re-derived for the private Git-dependency/tarball-into-a-separate-repo path described above, nor does it address AGPLv3 §13 (the network-interaction clause that is AGPLv3's actual point of departure from plain GPLv3 and does not require "redistribution" in the traditional sense to obligate source disclosure to a service's users). It also assumes `packages/extension-api` is effectively types/contracts-only; in fact it ships real runtime code (e.g. `register-extension.ts`, `manifest.ts`, the `semver`-based version-negotiation logic), not just ambient type declarations, which is a materially different fact pattern than a types-only package. Treat any of these three points — separate-repo consumption, AGPLv3 §13, and non-types-only runtime code — as unresolved until the "actual legal review" this caveat already calls for happens; do not extend today's engineering rationale to the SaaS/network-service case without that review.
...
**`rotations.status` enum (amended, Story 5.6):** `IN ('in_progress','staged','promoted','retired','completed','abandoned','stale_recovery','break_glass_complete')` — purely additive over the pre-5.6 set. New rotations are inserted as `staged` (not `in_progress`); `in_progress`/`completed` are retained only for historical-row compatibility (in-flight rows at the Story 5.6 migration's deploy time are migrated to `promoted`, never written as new-code steady state after that). The completion transaction described elsewhere in this document (`completeRotation()`'s single CAS `UPDATE` + `credential_versions.rotation_locked_at` clear) is now the **legacy** path, reachable only for genuinely `in_progress` rows. New rotations instead traverse two separate, equally atomic CAS transactions — `promoteRotation()` (`staged → promoted`, flips `credential_versions.promoted_at`) and `retireRotation()` (`promoted → retired`, cryptographically purges the old version and clears `rotation_locked_at`) — each its own explicit user action, per FR21/FR22's re-amended text.
