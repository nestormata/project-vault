...
5. **Task 10 e2e/Docker gap — resolved during pick-story verification (no longer open).** The original implementation left `@project-vault/mock-sso-extension` unreachable in a `pnpm deploy --prod` image (it's a devDependency, correctly pruned by default) with no way for `j6-sso-login.spec.ts` to actually pass against the Dockerized e2e stack. This was also masking a more basic bug: `pnpm-lock.yaml` gained the fixture as a workspace member, but `apps/api/Dockerfile`'s manual COPY list never listed its `package.json`, which broke `pnpm install --frozen-lockfile` for *every* Docker build, not just e2e. Both are fixed: the Dockerfile now always copies the fixture's `package.json`/source (required for `pnpm install` regardless of deploy target) and conditionally builds + grafts the built package into the deploy output behind a new `INCLUDE_MOCK_SSO_EXTENSION` build ARG (default `false` — production/default builds are unaffected), and `docker-compose.e2e.yml` sets that ARG to `'true'` for the e2e build only. Verified via two full `docker build` runs (default path: lockfile installs cleanly, no mock extension present; `--build-arg INCLUDE_MOCK_SSO_EXTENSION=true`: extension's `dist/`+`package.json` confirmed present at `node_modules/@project-vault/mock-sso-extension/`). The `pnpm --filter @project-vault/api sso:qa` manual-QA script remains usable as-is (runs via `tsx`, unaffected either way).
