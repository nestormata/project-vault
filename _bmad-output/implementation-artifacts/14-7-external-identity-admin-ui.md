...
- [ ] Task 1: `EXTERNAL_IDENTITY_UNLINKED` audit event (AC: 3, 6)
  - [ ] Write a failing test in `packages/shared/src/constants/audit-events.test.ts` (or the equivalent existing test file for this constants module) asserting `AuditEvent.EXTERNAL_IDENTITY_UNLINKED === 'external_identity.unlinked'`.
  - [ ] Add the constant to `packages/shared/src/constants/audit-events.ts`, alongside the existing `EXTERNAL_IDENTITY_LINKED`/`SSO_LOGIN_SUCCEEDED`/`SSO_LOGIN_REJECTED` trio (same Story-14.3-authored block).

- [ ] Task 2: `GET /api/v1/admin/external-identities` — list handler (AC: 1, 4, 5, 6, 8, 9, 10, 11)
  ...
  - [ ] Implement the handler inside `externalIdentityRoutes()`: `secureRoute(fastify, { method: 'GET', url: '/external-identities', security: { allowedRoles: ['admin'], requireMfa: true, writeAuditEvent: false, rateLimit: { max: 60, timeWindowMs: 60_000, key: 'GET /api/v1/admin/external-identities' } }, handler: ... })` — query `externalIdentities` `innerJoin(users, eq(users.id, externalIdentities.userId))` scoped to `secureCtx.auth.orgId`, ordered by `createdAt` descending (matching `listOrgUsers()`'s general query-shape convention, confirm actual ordering convention against a sibling list route during implementation).
  - [ ] Confirm (per AC-6) whether `route-exemptions.ts`'s `ROUTE_ACTION_CLASSIFICATIONS` needs a `'GET /api/v1/admin/external-identities'` entry by running `route-audit.test.ts`; add one (`action: 'read'`, `auditOmissionReason`) only if the test fails without it.

- [ ] Task 3: `DELETE /api/v1/admin/external-identities/:id` — unlink handler (AC: 3, 4, 5, 6, 7, 8, 9, 10, 11)
  ...
  - [ ] Implement the handler: `secureRoute(fastify, { method: 'DELETE', url: '/external-identities/:id', schema: { params: z.object({ id: z.string().uuid() }) }, security: { allowedRoles: ['admin'], requireMfa: true, writeAuditEvent: false, rateLimit: { max: 20, timeWindowMs: 60_000, key: 'DELETE /api/v1/admin/external-identities' } }, handler: ... })` — `DELETE ... WHERE id = :id AND org_id = secureCtx.auth.orgId RETURNING {...}`; zero rows returned → `404 { code: 'not_found' }`; on success, `writeHumanAuditEntry(secureCtx.tx, { eventType: AuditEvent.EXTERNAL_IDENTITY_UNLINKED, ... })`, then `200 { data: { id, userId, providerName, externalSubject } }`.

- [ ] Task 4: Web API client (AC: 1, 2, 3, 12)
  - [ ] New `apps/web/src/lib/api/external-identities.ts` (+ co-located `.test.ts`): `listExternalIdentities(fetchFn)`, `linkExternalIdentity(fetchFn, { userId, providerName, externalSubject })`, `unlinkExternalIdentity(fetchFn, id)` — thin `apiFetch<T>()` wrappers, matching `org-sso-domains.ts`'s pattern exactly.

- [ ] Task 5: `+page.server.ts` for `/settings/external-identities` (AC: 1, 4, 5, 12)
  ...
  - [ ] Implement: `requireUser(locals)` first, branch on `orgRole === 'admin'` exactly (owner excluded, per AC-4's judgment call) before calling `listExternalIdentities()`; also call `listOrgUsers()` (for the link form's member `<select>`) only when the role check passes.

- [ ] Task 6: `+page.svelte` for `/settings/external-identities` (AC: 1, 2, 3, 4, 5, 7, 12)
  - [ ] Render all five states (loaded-with-rows, empty, permission-denied, MFA-required, fetch-error), the Link-identity form (member `<select>` + provider/subject text inputs, disabled while submitting), and per-row Unlink button with `confirm()` + `busyKey` + `invalidateAll()`, following `/settings/sso-domains`'s exact structural precedent (closest sibling: full CRUD-shaped admin page).
  - [ ] Typed `ApiClientError` code-based error branches for the link form (`409 conflict`, `404 user_not_found`) and the unlink action (`404 not_found`) — never a generic `error.message` relay (this is exactly the High finding `14-6`'s code review caught and fixed — do not reintroduce it here).
  - [ ] Write a component test (`external-identities-page.test.ts`) asserting each state's distinguishing text and the create/unlink interaction flows with mocked API calls.

- [ ] Task 7: Settings index nav entry (AC: none directly — G3 navigation-truth requirement)
  - [ ] Add an **External Identities** `<li>` row to `apps/web/src/routes/(app)/settings/+page.svelte`, same markup shape as the existing rows, placed after SSO Domains (newest-addition-last convention `14-5`/`14-6` established).
  - [ ] Extend `settings-index-page.test.ts` with an assertion the new link/row renders and resolves to the correct href.

- [ ] Task 8: Tests and regression (AC: all)
  - [ ] `route-audit.test.ts` must pass with the two new routes correctly classified/exempted (AC-11) — run it explicitly.
  - [ ] `check-rls-coverage.test.ts` must still pass with `external_identities` un-excluded (AC-10).
  - [ ] `packages/db/src/__tests__/rls-isolation.test.ts` — confirm the existing read-only `external_identities` isolation test still passes; this story's own cross-org test lives in `external-identity-routes.test.ts` per AC-1 (API-level, not `packages/db`-level, since the route itself — not a raw query — is what must prove isolation here).
  - [ ] Full regression: `pnpm turbo typecheck lint test --filter=@project-vault/api --filter=@project-vault/web --filter=@project-vault/shared` green; confirm 80/80/80/80 coverage bar on all new/modified files.
  - [ ] `npx jscpd` against `(app)/settings/external-identities`, `(app)/settings/sso-domains`, `external-identities.ts`, `org-sso-domains.ts` — confirm no clone flags against the sibling pages this story deliberately mirrors.
  - [ ] Live-browser verification against a real `make docker-up` (or `pnpm dev`) stack with `@project-vault/mock-sso-extension` loaded (per this project's UI-story convention — memory: verify UI in Chrome, don't rely on test suites alone): link an identity, confirm it appears; attempt a duplicate link and confirm the `409` inline error; unlink an identity with the confirm dialog; verify the empty state; verify the permission-denied state as a non-admin role; verify the MFA-required state if reachable in the dev stack.
