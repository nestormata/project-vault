...
- [ ] [Review][Patch] Empty dependent-system name returns silently with no inline error [`apps/web/src/routes/(app)/projects/[projectId]/credentials/[credentialId]/+page.svelte` `onAddDependency`] — HTML `required` helps in browsers; programmatic submit still no-ops. Medium; left unfixed (not critical/high). Follow-up: `19-4-dependent-system-inline-validation`
