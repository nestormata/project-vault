...
- [ ] **Task 1: Close TOCTOU race + retire archive-guard seam** (AC-1)
  - [ ] Lock parent project row (or `isProjectArchived` re-check) in `initiateRotation`
  - [ ] Replace `findBlockingRotationIds`'s raw-SQL seam with a typed Drizzle query; delete `rotationsTableExists`
  - [ ] Confirm `archive-guards.test.ts`'s ADR-4.4-02 tripwire passes
  - [ ] Race + regression tests
- [ ] **Task 2: Flag single-actor self-attestation** (AC-2)
- [ ] **Task 3: Strengthen credential-reveal regression safety net** (AC-3)
- [ ] **Task 4: Break-glass idempotency protection** (AC-4)
- [ ] **Task 5: Handle NULL `initiatedBy` in stale-rotation notification** (AC-5)
- [ ] **Task 6: Handle malformed cron strings in dashboard aggregation** (AC-6)
- [ ] **Task 7: Bound org-dashboard upcoming-rotations computation** (AC-7)
- [ ] **Task 8: Add `org_id`-leading index for stale-detection job** (AC-8)
  - [ ] Migration (verify next-free number against `_journal.json`)
  - [ ] `check-rls` + query-plan verification
- [ ] **Task 9: Define audit-write failure handling in background jobs** (AC-9)
- [ ] **Task 10: Add CAS check to `abandon` transition** (AC-10)
- [ ] **Task 11: Pin down break-glass supersession audit-payload shape** (AC-11)
  - [ ] Update Story 5.3 file's AC-5/ADR-5.3-05 to remove the "implementer's choice" hedge
- [ ] **Task 12: Add missing fail-closed rollback tests** (AC-12)
- [ ] **Task 13: Add version IDs to `rotation.completed` audit payload** (AC-13)
- [ ] **Task 14: Full regression** — `pnpm turbo typecheck`, `pnpm turbo lint`, `pnpm jscpd` (0 clones), full `apps/api`/`packages/db`/`packages/shared` test suites, `pnpm --filter @project-vault/db check-rls`
