...
bmad-code-review adversarial pass (2026-07-25), diff `680f8c6..91f1078` (PR #182): Blind Hunter, Edge Case Hunter, Acceptance Auditor. Live-verified in a docker worktree: `POST /vault/init` with a fake KMS key ARN and no AWS credentials/network correctly returned `503 kms_unreachable` and left the vault uninitialized (real AC-3 behavior, not just a unit mock); passphrase-mode init/unseal exercised end-to-end through the web UI with no regression.

- [x] [Review][Patch] Reorder `kms` init: check already-initialized before calling `generateDataKey`, not after — every repeat `POST /vault/init` against an initialized kms vault currently burns a real, billed AWS `GenerateDataKey` call [apps/api/src/modules/vault/key-service.ts] (resolved decision D1) — fixed, plus AC-6's test updated to assert `generateDataKey` is no longer called at all on a repeat init
- [x] [Review][Patch] Write an audit-log entry for KMS init/unseal failures (`kms_permission_denied`/`kms_unreachable`/`kms_key_unavailable`) — currently indistinguishable from routine validation noise in the audit trail [apps/api/src/modules/vault/key-service.ts] (resolved decision D2) — fixed: `KMS_PERMISSION_DENIED` now logs at `error` level (vs `warn` for other codes) in `routes.ts`'s existing interim operational-logging audit trail (Story 9.1 D6 pattern); a full encrypted `audit_log_entries` write isn't architecturally possible pre-unseal (no `_auditKey` yet), same constraint every other vault mode already has
- [x] [Review][Patch] Hard-fail startup if `VAULT_KMS_ENDPOINT` is set with `NODE_ENV=production` — currently only a code comment prevents an accidental/malicious KMS-traffic redirect (SSRF/key-diversion risk) [apps/api/src/config/env.ts] (resolved decision D3) — fixed, mirrors this file's other FATAL production guards; 2 new tests
- [x] [Review][Patch] `deriveIkmForUnseal`'s corruption guard only checks `kmsEncryptedDek === null` for `kms_type='kms'` — an unrecognized/inconsistent `kms_type` value (or `kmsKeyId` null while `kmsEncryptedDek` isn't) silently falls through to the file-mode branch and produces a misleading `INVALID_KEY_FILE` error instead of a clear corruption signal [apps/api/src/modules/vault/key-service.ts] — fixed: any `kmsType !== 'file'` now throws `VAULT_CORRUPTED` before reaching the file-mode branch
- [x] [Review][Patch] `AwsKmsProvider.generateDataKey`/`decryptDataKey` never validate the returned plaintext buffer is 32 bytes — malformed/wrong-length key material would be silently used as IKM [apps/api/src/modules/vault/kms-provider.ts] — fixed, both methods now reject a non-32-byte plaintext
- [x] [Review][Patch] AWS `ValidationException`/`InvalidCiphertextException` are misclassified as `unknown` → `503 kms_unreachable`, giving operators a false "retry" signal for what are actually permanent failures [apps/api/src/modules/vault/kms-provider.ts] — fixed, both added to the `not_found`-equivalent category
- [x] [Review][Patch] A non-`KmsProviderError` thrown before the SDK `send()` call (e.g. provider/client construction failure) bypasses the documented `KMS_*` `AppError` contract and could surface a generic 500 [apps/api/src/modules/vault/key-service.ts] — fixed, both `mapKmsErrorForInit`/`mapKmsErrorForUnseal` now treat a non-`KmsProviderError` as `'unreachable'` instead of rethrowing raw
- [x] [Review][Patch] `VAULT_KMS_ENDPOINT` accepts any string — no URL/scheme validation, so a malformed value only fails at KMS-call time with an opaque SDK error [apps/api/src/config/env.ts] — fixed, restricted to `http://`/`https://`; 1 new test
- [x] [Review][Patch] Stored `kms_encrypted_dek` is never validated as well-formed base64 before `Buffer.from` decode — malformed/corrupted data silently mis-decodes instead of failing with a clear corruption error [apps/api/src/modules/vault/kms-provider.ts] — fixed, `decryptDataKey` now validates base64 shape before decode
- [x] [Review][Patch] The plaintext KMS data key (`ikm`) is never zeroed after use, unlike the file/envelope modes' explicit `.fill(0)` discipline elsewhere in this same diff [apps/api/src/modules/vault/key-service.ts] — verified false positive: `deriveAllKeysFromIkm()`'s `ikm.fill(0)` (key-service.ts) zeroes this exact buffer on every call path (success and ALREADY_INITIALIZED-conflict alike), same as the file/envelope IKM. No change needed.
- [x] [Review][Patch] AC-8's required init-side negative test (extra `passphrase` field on a `kms` init body is ignored) is missing — only the unseal-side equivalent (AC-10) is tested [apps/api/src/__tests__/vault-kms-lifecycle.test.ts or vault-kms-errors.test.ts] — added
- [x] [Review][Patch] AC-17's required KMS-specific regression test (`kms` init without bootstrap token → `403 bootstrap_forbidden` before any AWS call) is missing [apps/api/src/__tests__/vault-kms-errors.test.ts] — added, also asserts `generateDataKey` was never called
- [x] [Review][Patch] AC-18's required positive/negative log-redaction tests (KMS ARN visible in `vault.init` log; ciphertext/plaintext never logged) are missing for `kms` mode — `vault-operational-logging.test.ts`/`vault-log-redaction.test.ts` were not touched by this diff [apps/api/src/__tests__/vault-operational-logging.test.ts, vault-log-redaction.test.ts] — added 3 tests total across both files
- [x] [Review][Patch] `docs/runbook.md`'s new KMS section says "credential rotation is transparent by construction" without clarifying this covers IAM credential rotation only, not actual data-key rotation (no such execution path exists yet, per the pre-existing `key_rotated_at` gap) [docs/runbook.md] — fixed, clarifying sentence added
- [x] [Review][Patch] `README.md`'s "Vault unsealing" row is now unconditionally `✅ Done` with no caveat, while two known v1 limitations (deferred DB-level CHECK, no key-rotation execution path) remain undisclosed there, inconsistent with this project's stated "honest disclosure" documentation discipline [README.md] — fixed, KMS data-key rotation gap added to the existing "Current limitations" list

All 15 patches applied and verified: full `apps/api` typecheck clean, targeted vault/env/kms test files green (165 tests, including 6 new/updated), `eslint` clean on every touched file (0 errors). Full `apps/api` suite run twice (2221 tests): both runs show the identical 18-file/~77-test failure set, entirely in unrelated modules (workers, compliance/erasure, invitations, recovery, monitoring/status-page, machine-users, org, platform-admin) — zero vault/kms/env files among them. Consistent with this project's documented pre-existing full-suite DB-state pollution flakiness; not caused by this review's changes.
- [x] [Review][Defer] Deferred DB-level `CHECK` constraint for `kms_type='kms'` requiring non-null `kms_encrypted_dek` [packages/db/src/migrations/0048_vault_kms_columns.sql] — deferred, explicitly accepted trade-off in the story's own AC-7, mitigated by the existing app-level `VAULT_CORRUPTED` guard
- [x] [Review][Defer] `kmsKeyId` has minimal input validation (`z.string().min(1)` only, no ARN shape/length checks) [apps/api/src/modules/vault/schema.ts] — deferred, low severity, AWS itself rejects malformed key IDs at call time
- [x] [Review][Defer] `__setKmsProviderForTest` is exported unconditionally from the production module, no env gate [apps/api/src/modules/vault/key-service.ts] — deferred, mirrors the existing `__getRawBackupKeyForTest` precedent already established in this codebase
- [x] [Review][Defer] `VaultUnsealRequestSchema`'s Zod refine was relaxed from "exactly one" to "at most one" field for all modes, not scoped to `kms` only — per-branch checks in `deriveIkmForUnseal` now carry required-field enforcement instead [apps/api/src/modules/vault/schema.ts] — deferred, verified safe today (AC-10 negative test), theoretical risk for a future mode that forgets its own guard
- [x] [Review][Defer] No proactive custody-risk alert for a weak/overly-broad KMS key policy or a key entering pending-deletion [apps/api/src/workers/key-custody-check.ts] — deferred, explicitly out of v1 scope per the runbook's own disclosed limitation
- [x] [Review][Defer] New test files hardcode a plaintext test DB connection string rather than a shared constant [apps/api/src/__tests__/vault-kms-errors.test.ts, vault-kms-lifecycle.test.ts] — deferred, matches the existing convention already used across this test suite
