...
| `org_sso_domains` admin UI + write API (create/edit/remove domain-to-provider mappings; currently seeded directly, same as `sso-qa.ts`) | 14.4 | API-only — no POST/PATCH/PUT route exists for `org_sso_domains` anywhere in `apps/api/src`, only the read-only `domain-lookup-routes.ts`; `14-4`'s own Dev Notes explicitly compared this to the `14-2` gap above but it was never added to this table until the Epic 14 closure retro found it | **Open — scheduled 2026-07-27 (Epic 14 closure retro)** as `14-6-org-sso-domains-admin-ui` (backlog, `sprint-status.yaml`). See `epic-14-retro-2026-07-27.md` Finding 2. |
