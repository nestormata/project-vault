...
| SSO login button / email-first routing UI (login screen entry point for the registered external-provider strategy) | 14.3 | API-only — `14-3-authenticate-via-a-registered-external-provider-strategy` makes the SSO login flow callable, but no story in the current sprint backlog builds the login screen's SSO button/email-first routing; `14-4-route-login-to-sso-by-email-domain` (backlog) is the next story and owns this UI, but is not yet implemented | **Open — must be scheduled** — `epic-14` should not move to `done` until 14.4 (or an equivalent UI story) ships and closes this gap |
