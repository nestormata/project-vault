...
| 2026-07-27 | bmad-code-review adversarial pass (Blind Hunter/Edge Case Hunter/Acceptance Auditor): no critical/high findings. One low finding left as documented follow-up (GET /me's response schema hardcodes the locale enum instead of deriving from SUPPORTED_LOCALES) | Claude Sonnet 5 |
| 2026-07-27 | Rebased onto origin/main; migration renumbered 0055 -> 0056 due to a collision with Story 13.4's own 0055 (merged to main first) | Claude Sonnet 5 |
| 2026-07-27 | Found and fixed a pre-existing, unrelated bug while chasing a `make ci` failure: `backup-snapshot.test.ts`'s "latest schedule run" query had no ORDER BY/LIMIT, so it could pick an arbitrary historical row from the shared dev DB instead of the row the test just created — confirmed deterministic on a fresh origin/main checkout before fixing; fixed via `ORDER BY startedAt DESC LIMIT 1` (commit 6145af7) | Claude Sonnet 5 |
| 2026-07-27 | `make ci` fully green; PR #231 opened | Claude Sonnet 5 |
| 2026-07-27 | Live Chrome verification against a fresh `make docker-up` stack (vault init, user registration/login, Settings > Language) found a real bug the test suite couldn't catch: the `updateLocale` form action 403'd every time under docker compose, even though the underlying `PATCH /api/v1/users/me/locale` endpoint worked fine via direct curl. Root cause: `@sveltejs/adapter-node` needs an `ORIGIN` env var for its CSRF same-origin check, and `docker-compose.yml`'s `web` service never set one — this is the first SvelteKit form action anywhere in the app (login/register bypass this path via direct client-side API calls), so the gap was never exercised before this story. Fixed by adding `ORIGIN: http://localhost:${WEB_HOST_PORT:-5173}` to the `web` service, matching the browser-facing host/port convention already used by the `api` service's `CORS_ALLOWED_ORIGINS` (commit 138c8a8). After the fix: reload-free locale switching, DB persistence, and cross-session SSR-cookie restoration (a fresh login correctly rendered the Settings page in Spanish, `Idioma`/`Selección actual`, with no user action) all verified working live in Chrome | Claude Sonnet 5 |
