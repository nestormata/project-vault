...
- **Decision implemented in this story:** a single `SECURITY DEFINER` Postgres function, owned by the migration-runner role (not `vault_app`), is the only sanctioned deletion path:
...
-- vault_app is granted EXECUTE on the function ONLY — never a raw DELETE grant.
GRANT EXECUTE ON FUNCTION purge_expired_audit_log_entries(uuid, timestamptz) TO vault_app;
```

  - The retention worker (`apps/api/src/workers/audit-retention-prune.ts`, new) calls this function via `tx.execute(sql`SELECT purge_expired_audit_log_entries(${orgId}, ${cutoff})``)` — one call per org with a configured `retentionDays`, never a raw `DELETE`/`.delete()` Drizzle call against `auditLogEntries`.
...
### D4 — Outbound webhook fetch needs SSRF protection; none exists anywhere in this codebase today
...
- **Decision implemented in this story:** new shared utility `apps/api/src/lib/safe-fetch.ts` exporting `safeFetchExternal(url, init, opts)`:
  ...
  - This utility is intentionally placed in `apps/api/src/lib/` (general infra, not `modules/audit/`) since it is the correct home for any future outbound-integration feature that needs the same SSRF protection — but no other consumer exists yet in this story; do not over-generalize its API beyond what this story needs.
  - S3 forwarding (D-schema below) does **not** need this utility — the AWS SDK talks to a configured `bucket`/`region`/`endpoint` the org admin also controls, but S3-compatible object storage over a signed SDK client is a materially different trust boundary (it requires valid AWS-style credentials to do anything, unlike a bare webhook POST) and typical deployments point it at a known, operator-trusted S3/Minio endpoint. Document this scope boundary explicitly rather than silently leaving S3 unprotected without comment: if a future story needs to harden the S3 endpoint against SSRF too (e.g., a malicious `s3Endpoint` pointed at an internal service), that is a reasonable follow-up, not a gap this story silently introduces without flagging.
...
| epics.md: retention job "prunes rows older than retention window" | `SECURITY DEFINER` function `purge_expired_audit_log_entries()`, invoked only by the retention worker (D2) | A plain `DELETE` is blocked by Story 8.1's own append-only trigger + grant revoke; this is the narrow, auditable exception |
...
| epics.md: config `{ url?, secretHeader?, bucket?, prefix?, region?, accessKeyId?, secretAccessKey? }` | Same fields, plus `endpoint?` (new, optional) for S3-compatible/Minio support | epics.md's own AC-E8b requires Minio support, which needs a configurable endpoint the literal field list omits |
...
| `PUT /audit/forwarding` | `minimumRole: 'admin'`, `requireMfa: true`. Configures webhook or S3 forwarding; secrets encrypted at rest; SSRF-guarded webhook URL validation. |
| `PUT /audit/retention` | `minimumRole: 'admin'`, `requireMfa: true`. Sets `retentionDays` within platform bounds (D7). |
| Webhook delivery | Every-minute watermark-cursor catchup cron (D3); auto-disables after repeated failure. |
| S3 delivery | Daily gzipped-JSONL batch cron per org. |
| Retention pruning | Daily cron calling the SECURITY DEFINER purge function (D2), org-scoped. |
...
**Edge case — SSRF-blocked URL (D4):** **Given** `config.url = "http://169.254.169.254/latest/meta-data/"` (cloud metadata endpoint) or `"https://localhost:8080/internal"` or a hostname that resolves to `10.0.0.5`, **when** the same `PUT` is called, **then** the response is `422 { code: "unsafe_forwarding_url", message: "..." }` — rejected either at Zod-schema level (non-`https://` scheme) or at `safeFetchExternal`'s DNS-resolution check (private/loopback range), tested independently for: `http://` scheme, loopback (`127.0.0.1`), link-local (`169.254.169.254`), private RFC1918 (`10.0.0.5`, `172.16.0.1`, `192.168.1.1`), and a control case confirming a normal public HTTPS URL is accepted.
...
### AC-19: Forwarding — S3 Batch Delivery (Daily, Gzipped JSONL, Minio-Compatible)

**Given** S3 forwarding is configured (`{ type: "s3", config: { bucket: "compliance-bucket", prefix: "org-abc/", region: "us-east-1", accessKeyId: "AKIA...", secretAccessKey: "..." } }`),

**When** the daily `audit:s3-forward-daily` cron runs (e.g. `0 1 * * *`, UTC),

**Then** it queries the previous UTC day's rows for that org, serializes each as one JSON line (JSONL), gzips the batch, and uploads via `@aws-sdk/client-s3`'s `PutObjectCommand` to `s3://compliance-bucket/org-abc/2026-07-03.jsonl.gz` (prefix + ISO date + fixed suffix); `secretAccessKey` is decrypted transiently via `withSecret()` immediately before constructing the S3 client credentials and never logged.

**Edge case — Minio/custom-endpoint config:** **Given** `config.endpoint = "https://minio.internal.example.com:9000"` is additionally supplied (this story's D9 extension beyond epics.md's literal field list), **when** the upload runs, **then** the S3 client is constructed with `{ endpoint, forcePathStyle: true, region, credentials }` — verified with a mocked S3-compatible test double (e.g. an in-memory mock of the SDK client, not a live Minio instance in CI).

**Edge case — zero rows for the day:** if an org has zero audit rows for the previous UTC day, **no object is uploaded** (an empty gzip file would be misleading — "we checked, there was nothing" is different from "we forgot to check"; this is logged at debug level, not treated as an error).
...
- [ ] Task 1: Migration `0030` (AC: 24, D2, D5)
  ...
  - [ ] 1.3 Write migration SQL: three `CREATE TABLE`s + RLS enable + isolation policies (exact pattern from `0029`), `CREATE OR REPLACE FUNCTION purge_expired_audit_log_entries(...)`, `CREATE OR REPLACE FUNCTION prevent_audit_log_mutation()` (amended per D2), `GRANT EXECUTE ... TO vault_app`
...
- [ ] Task 4: Webhook forwarding config + SSRF-safe delivery (AC: 17, 18, 20, 21)
  - [ ] 4.1 `apps/api/src/lib/safe-fetch.ts` — `safeFetchExternal()`: HTTPS-only, DNS-resolution private-range rejection, timeout, bounded response read (D4); unit tests for each blocked-range case
  - [ ] 4.2 `apps/api/src/modules/audit/forwarding.ts` — `configureForwarding()` (upsert, encrypts secrets via `encryptValue()`), `runWebhookForwardCatchup()` (watermark cursor, bounded-failure auto-disable, D3)
  - [ ] 4.3 `PUT /audit/forwarding` route: `minimumRole: 'admin'`, `requireMfa: true`, never echoes secrets
...
- [ ] Task 5: S3 forwarding (AC: 19)
  ...
  - [ ] 5.2 `apps/api/src/modules/audit/s3-forward.ts` — daily batch job: query prior-UTC-day rows, JSONL + gzip, `PutObjectCommand`, Minio-compatible `endpoint`/`forcePathStyle` support
