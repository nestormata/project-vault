...
3. **Role-gate decision required, not assumed**: the existing `PUT /api/v1/projects/:projectId/status-page` route this story reuses is gated at `minimumRole: 'member'`, not admin/owner. Default to (a) reorder stays available to project members (matching the existing route's actual gate, no backend change) — this requires no special sign-off, it's simply building UI consistent with the route it calls. Only if a deliberate decision is made to (b) tighten reorder to admin/owner does that require explicit confirmation before implementation, since it changes an existing endpoint's authorization. Do not build UI against an assumed role gate that contradicts the route it calls.
...
- [ ] Task 1: Confirm the existing `PUT` route/service wiring still matches this description; no new route expected (AC: 1)
- [ ] Task 2: Decide and document the reorder role gate (AC: 3)
- [ ] Task 3: Build keyboard-operable reorder UI on the status-page admin screen (AC: 2, 6, 7)
- [ ] Task 4: Wire persistence + public-page reflection (AC: 4)
- [ ] Task 5: Tests incl. concurrency and cross-tenant behavior (AC: 5, 8)
