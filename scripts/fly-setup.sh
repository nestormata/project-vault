...
# secret the api/web apps need at boot. Safe to re-run: `fly apps create`/`fly volumes
# create`/`fly secrets set` are all idempotent or explicitly guarded below.
#
# Usage: FLY_ORG=personal ./scripts/fly-setup.sh
#
# Requires: flyctl authenticated (`fly auth login`), openssl.
set -euo pipefail
...
ORG="${FLY_ORG:?Set FLY_ORG to your Fly.io org slug (fly orgs list)}"

command -v fly >/dev/null 2>&1 || { echo "flyctl not found — see https://fly.io/docs/flyctl/install/" >&2; exit 1; }

echo "== Creating apps (region=${REGION}, org=${ORG}) =="
for app in "$DB_APP" "$API_APP" "$WEB_APP"; do
  fly apps create "$app" --org "$ORG" 2>&1 | grep -v "Name has already been taken" || true
done

echo "== db: volume =="
fly volumes list -a "$DB_APP" --json 2>/dev/null | grep -q project_vault_demo_db_data \
  || fly volumes create project_vault_demo_db_data -a "$DB_APP" --region "$REGION" --size 3 --yes
...
echo "== db: deploy =="
fly deploy -c fly.db.toml -a "$DB_APP" --ha=false
